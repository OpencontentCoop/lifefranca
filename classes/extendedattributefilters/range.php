...
class LifeFrancaRangeExtendedAttributeFilter implements eZFindExtendedAttributeFilterInterface, SearchResultDecoratorInterface
{
	public function filterQueryParams( array $queryParamList, array $filterParams )
    {
    	if ( !empty( $filterParams['field'])
            ...
            $queryParamList[$perFieldRangePrefix . '.gap'] = $filterParams['gap'];
    ...
    public function decorate(SearchResults $searchResults, $rawResults)
    {
        ...
        if (isset($result['facet_counts']['facet_ranges'])) {
            $searchResults->range = $result['facet_counts']['facet_ranges'];
        }
    }
}