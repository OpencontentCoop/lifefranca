;(function ($, window, document, undefined) {
    ...
    var TypeFilterFactory = function(label, queryField, containerSelector, cssClasses){
        return {
    ...
    $.fn.lifeFrancaBlock = function (settings) {
                    ...
                    if (currentValues.length && jQuery.inArray('all', currentValues) == -1) {                        
                        ...
                        if (filter.type == 'layer'){
                            if (currentTypes.length > 0){
                                $.each(currentTypes, function(){
                                    var type = this;
                                    $.get('/openpa/data/lifefranca', {type: options.dataChart, field: field, value: currentValues, types: [type]}, function(response) {
                                        response.tooltip = {
                                        ...
                                        var currentFilterAggrItem = $(chartAggrTpl.render({
                                            label: (response.xAxis.categories[0]) ? response.xAxis.categories[0] : type,
                                            color: filter.layerOptions.color,
                                            height: 200 + (currentValues.length * 10)
                                        }));                  
                                        currentFilterAggrItem.find('a.close').on('click', function(e){
                                            $('a[data-value="'+type+'"]', typeFilter.container).trigger('click');
                                            e.preventDefault();
                                        });      
                            ...
                            }else if($('#null-types').length == 0){
                                currentFilterContainer.append('<li id="null-types"><em>Seleziona una o più tipologie</em></li>'); 
                            }
                        }
                    }else{
        ...
        if (options.layers.length > 0){
            ...
            searchView.addFilter(TypeFilterFactory(
                'Tipologia',
                'tipologia', 
                '#'+that.attr('id')+' ul[data-filter="tipologia"]', 
                options.cssClasses
            ));
